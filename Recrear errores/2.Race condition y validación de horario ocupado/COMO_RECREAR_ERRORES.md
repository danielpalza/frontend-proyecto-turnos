# Cómo Recrear Errores - Race Condition y Validación de Horario Ocupado

Este documento proporciona instrucciones paso a paso para recrear y probar todos los casos relacionados con race conditions y validación de horario ocupado.

**Referencia**: `2.Race condition y validación de horario ocupado.md`
...
### Método 2: Simular Múltiples Clics Rápidos
### Método 3: Usar Herramientas de Desarrollo (Postman, cURL)
### Método 4: Modificar el Backend Temporalmente
...
**Ubicación del código**: `AppointmentService.create()`  
**Endpoint**: `POST /api/appointments`  
**Código HTTP esperado**: `409 Conflict`

#### Escenario 1: Horario ya existe en la base de datos
...
#### Escenario 2: Usando Postman/cURL
...
**Ubicación del código**: `AppointmentService.update()`  
**Endpoint**: `PUT /api/appointments/{id}`  
**Código HTTP esperado**: `409 Conflict`

#### Escenario: Actualizar turno a un horario que ya está ocupado
...
**Ubicación del código**: `AppointmentService.partialUpdate()`  
**Endpoint**: `PATCH /api/appointments/{id}`  
**Código HTTP esperado**: `409 Conflict`

#### Escenario: Cambiar solo la hora a un horario ocupado
...
## 📋 Casos de Prueba - Race Condition

### 4. Múltiples Clics Rápidos en "Guardar"

**Ubicación del código**: `turnos-view.component.ts:140-145`  
**Protección**: Botón deshabilitado + flag `isLoading`

#### Escenario: Usuario hace clic múltiples veces rápidamente

**Pasos para recrear**:
1. Abrir el diálogo de crear turno
2. Llenar todos los campos
3. Hacer clic rápidamente 5 veces en "Guardar" (antes de que se deshabilite)

**Resultado esperado**:
- ✅ Solo se crea 1 turno (no 5)
- ✅ El botón se deshabilita después del primer clic
- ✅ Se muestra "Guardando..." en el botón
- ✅ Los clics adicionales son ignorados

**Verificación**:
- Revisar la base de datos: solo debe haber 1 turno creado
- Revisar la consola del navegador: solo debe haber 1 petición HTTP

---

### 5. Crear Paciente con DNI Duplicado

**Ubicación del código**: `PatientService.create()`  
**Endpoint**: `POST /api/patients`  
**Código HTTP esperado**: `409 Conflict`

#### Escenario: Intentar crear paciente con DNI existente

**Pasos para recrear**:
1. Crear un paciente con DNI "12345678"
2. Intentar crear otro paciente con el mismo DNI desde el diálogo de turnos

**Resultado esperado**:
- ✅ Backend devuelve `409 Conflict`
- ✅ Mensaje: "Ya existe un paciente con este DNI: 12345678"
- ✅ Frontend muestra notificación toast roja
...
### 6. Race Condition con Múltiples Usuarios

**Escenario**: Dos usuarios intentan crear turno en el mismo horario simultáneamente
...
2. Ambos usuarios seleccionan:
   ...
   - Misma fecha
   - Misma hora
3. Ambos hacen clic en "Guardar" casi simultáneamente

**Resultado esperado**:
- ✅ Solo uno de los turnos se crea exitosamente
- ✅ El otro recibe error `409 Conflict`
- ✅ Si se aplica el constraint de BD, el segundo falla a nivel de base de datos

**Nota**: Este escenario es difícil de recrear manualmente. Se recomienda usar herramientas de carga como Apache JMeter o scripts automatizados.
...
### Race Conditions
- [ ] Múltiples clics en "Guardar" → Solo se crea 1 turno
- [ ] Botón se deshabilita durante carga → Verificar visualmente
- [ ] Flag isLoading previene múltiples submits → Verificar en consola
- [ ] Crear paciente con DNI duplicado → Debe devolver 409 Conflict
- [ ] Múltiples usuarios crean turno simultáneamente → Solo 1 debe tener éxito

### Protección Frontend
- [ ] Botón "Guardar" deshabilitado cuando `isLoading` es `true`
- [ ] Botón muestra "Guardando..." durante el proceso
- [ ] Botón "Cancelar" deshabilitado durante carga
- [ ] Diálogo NO se cierra en caso de error (crear paciente/turno)
...
3. ✅ El mensaje es claro y específico:
   ...
   - "Ya existe un paciente con este DNI: {dni}"
...
Para probar race conditions reales, se puede usar un script que haga múltiples peticiones simultáneas:
...
| DNI duplicado | 409 Conflict | "Ya existe un paciente con este DNI..." | Permanece abierto | ❌ No |
| Sin conexión | 0 | "No se pudo crear el turno..." | Permanece abierto | ❌ No |
| Múltiples clics | 201 Created | "Turno creado correctamente" | Se cierra | ✅ 1 solo |

---

## 📚 Referencias

- **Documento de implementación**: `2.Race condition y validación de horario ocupado.md`
- **Resumen de cambios**: `bakend-proyecto-turnos/RESUMEN_CAMBIOS_VALIDACION_HORARIO.md`
- **Análisis original**: `ANALISIS_ERRORES_TURNOS_VIEW.md` (Punto 2)
