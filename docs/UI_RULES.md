...
Breakpoints y mixins (`$breakpoint-*`, `@include mobile-xs/mobile/tablet/desktop-sm`) documentados en [DESIGN_SYSTEM.md](./DESIGN_SYSTEM.md#espaciado-radios-sombras-breakpoints-z-index). Lo que sigue son las convenciones que se fueron repitiendo al ajustar vistas para mobile (Panel, Turnos, Seguimiento, Configuraciones, Navbar):

- **Un bloque `@include mobile { ... }` por componente, al final del archivo.** Varios componentes ya traían `@include tablet` (≤991px) y/o `@include mobile-xs` (≤575px); los ajustes específicos de mobile (≤767px) se agregan en un bloque nuevo **después** de esos, para ganar la cascada en el rango donde se solapan (ej. `month-calendar.component.scss`, `turnos-view.component.scss`, `panel-view.component.scss`). Si una regla de `mobile-xs` queda contradicha por el nuevo bloque, se borra en vez de dejarla muerta.
- **Truncado con ellipsis**: no hay una clase utilitaria compartida (`.text-truncate` no existe en `styles.scss`). Se repite a mano el trío `white-space: nowrap; overflow: hidden; text-overflow: ellipsis;` en cada lugar que lo necesita (`.navbar-user-name-text`, `.metric-value` en Panel, `.tile-value` en Turnos) — si aparece un cuarto/quinto caso, vale la pena extraerlo a un mixin en `_mixins.scss`.
- **Texto completo vs. abreviado según breakpoint**: cuando hay que reemplazar un texto por una versión corta solo en mobile (no ocultarlo, *cambiarlo*), el patrón es declarar dos `<span>` en el HTML — uno con el texto completo, otro con la versión corta — y alternar su `display` por CSS dentro de `@include mobile`. Ya existía en `navbar.component.html` (`.navbar-user-icon` vs `.navbar-user-name-text`, aunque hoy el ícono quedó sin uso — ver nota abajo) y se reutilizó en `panel-view.component.html` para los headers de la tabla de profesionales (`.th-text-full` "Facturación"/"Tasa de completitud" vs `.th-text-short` "$"/"%"). Es CSS puro, no toca el `.ts`.
- **Reordenar elementos que cruzan el límite de un componente Angular**: cuando dos elementos que deben quedar visualmente adyacentes en mobile viven en componentes Angular distintos (ej. `daily-summary` dentro de `<app-month-calendar>` y `turnos-panel-section` en `TurnosViewComponent`), no hace falta `::ng-deep` (que además casi no se usa en el repo — un solo caso en `app.scss`). Se colapsan los wrappers intermedios con `display: contents` (uno desde el componente padre, apuntando al *host* del hijo — eso sí es válido sin `::ng-deep` — y el resto desde el propio `.scss` del componente hijo, sobre sus propios elementos) para que los nodos reales pasen a ser ítems directos del grid/flex del ancestro común, y se los reordena con `order`. Ver `turnos-view.component.scss` + `month-calendar.component.scss`.
  - **Gotcha**: un elemento con `display: contents` no genera caja, así que cualquier `min-width`/`width` que tuviera deja de aplicarse. Si el ancestro es un CSS Grid de una sola columna (`grid-template-columns: 1fr`), sin ese freno el track puede crecer hasta el `min-content` de los hijos promovidos y desbordar el viewport. Hay que trasladar el `min-width: 0` a los elementos que quedan como ítems reales del grid, y/o usar `grid-template-columns: minmax(0, 1fr)` en vez de `1fr` a secas.
- **Errores de un breakpoint que no resetea todo lo que tocó el desktop**: la causa de que Seguimiento fuera inutilizable en mobile no era falta de responsive — era un `margin: 0 12rem` puesto para desktop que el `@media (max-width: 768px)` existente nunca reseteaba (solo ajustaba el `padding`). Al agregar/editar un breakpoint, conviene repasar *todas* las propiedades de layout del selector que se está tocando (no solo las que "parecen" responsive), no solo las nuevas que se están agregando.
