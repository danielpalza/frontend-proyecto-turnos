...
| `src/testing/auth-service.mock.ts` | **Nuevo (2026-08-28)**. `createAuthServiceMock(overrides?)` — mock compartido y tipado de `AuthService`, primer helper de este tipo en el repo (ver § 9, entrada 2026-08-28). Excluido del build de producción vía `tsconfig.app.json` (`exclude: ["src/testing/**"]`), incluido en `tsconfig.spec.json` (`include: ["src/testing/**/*.ts"]`). |
...
| **`ng-mocks` y MSW evaluados y descartados por ahora** | Ver § 2. **Matiz 2026-08-28**: se agregó `src/testing/auth-service.mock.ts` (§ 4) para `AuthService` específicamente — es un factory propio de ~35 líneas, no la adopción de `ng-mocks`; no reabre la decisión de § 2, resuelve un riesgo puntual (ver § 9) sin la dependencia externa. | Reconsiderar `ng-mocks` si el mismo problema (mock manual desactualizado tras un método nuevo) aparece en otro servicio con superficie amplia y un factory a mano deja de alcanzar. |
| ~~**26 specs mockean `AuthService` a mano, sin relación de tipos con la clase real**~~ — **Resuelto (2026-08-28)** | Cada spec definía su propio objeto literal para `AuthService`, sin ningún tipo compartido — el mecanismo exacto que causó el incidente de `hasRole` (fila de arriba, 2026-08-28) podía repetirse con el próximo método nuevo, en cualquiera de los 26. | `src/testing/auth-service.mock.ts`: `createAuthServiceMock(overrides)` con defaults para los 16 miembros públicos de `AuthService`, devuelto vía `Proxy` — cualquier método no contemplado explícitamente responde con un `vi.fn()` no-op en vez de `undefined` explotando. Los 26 specs migrados, suite verificada en 894/894 verde tras la migración. |
| ~~**`appointments-panel.component.spec.ts`: el test de doble-submit no ejercía el guard**~~ — **Resuelto (2026-08-28)** | `'evita doble submit mientras hay uno en vuelo'` usaba un observable síncrono (`of(...).pipe()`) que resolvía en el mismo tick — nunca había una ventana "en vuelo" que el guard `addingPayment.has(cardId)` pudiera bloquear; el test pasaba igual aunque se borrara el guard. | Alineado al patrón ya usado en 7 specs hermanos (`profesionales-panel.component.spec.ts:320`, etc.): `mockReturnValue(new Subject<Appointment>())` + dos invocaciones seguidas + `toHaveBeenCalledTimes(1)`. |
...
- **2026-08-28** — Fix de los 15 tests en rojo (commit `7ffedb7`, "fix tests"), documentado desde la
  ...
  nuevos rotos.
- **2026-08-28 (segunda ronda)** — Dos fixes de una auditoría de calidad de tests (no de cobertura):
  (1) `appointments-panel.component.spec.ts` — el test de doble-submit de pagos no ejercía el guard
  real (usaba un observable síncrono que resolvía en el mismo tick), reescrito con el patrón `Subject`
  pendiente + doble invocación ya usado en 7 specs hermanos (§ 8); (2) se creó
  `src/testing/auth-service.mock.ts` (`createAuthServiceMock`, § 4) y se migraron los 26 specs que
  mockeaban `AuthService` a mano, cerrando el riesgo estructural que causó el incidente de `hasRole`
  (entrada anterior) — cualquier método no contemplado en los defaults del factory ahora responde con
  un `vi.fn()` no-op vía `Proxy` en vez de romper el test. Requirió un ajuste de configuración:
  `tsconfig.spec.json` no incluía `src/testing/**` por defecto (el builder de Angular no lo agrega
  transitivamente como haría `tsc` puro) y `tsconfig.app.json` sí lo hubiera arrastrado al build de
  producción sin excluirlo explícitamente — ambos `tsconfig.*.json` actualizados. Verificado con
  `npm run test:coverage` (**894/894 tests, 75/75 archivos, sin cambios en el conteo**) y
